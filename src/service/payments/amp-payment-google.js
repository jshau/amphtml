...
export class AmpPaymentGoogleIntegration {
  ...
  startInlinePayment(element) {
    ...
    this.iframeService_ =
        getServiceForDoc(this.ampdoc_.win.document, SERVICE_TAG);
  ...
  startGpayButton(element) {
    ...
    this.viewer_ = Services.viewerForDoc(this.ampdoc_);
    this.actions_ = Services.actionServiceForDoc(element);
    return this.viewer_.whenFirstVisible()
  ...
  renderButton_(onClickFunc) {
    this.activeElement_.appendChild(createButtonHelper({
      onClick: onClickFunc,
    }));
   ...
   */
  localOnClickButton_() {
    this.client_.loadPaymentData(this.getPaymentDataRequest_())
        .then(data => this.triggerAction_(data));
  }
  ...
  triggerAction_(paymentData) {
    ...
    const event =
        createCustomEvent(this.ampdoc_.win, `${TAG}.${name}`, paymentData);
  ...
  renderInlinePayment_(iframeSrc) {
    if (this.iframe_) {
      ...
      this.ampdoc_.win.addEventListener(
          'message', event => this.onMessage_(event));
    ...
    const enclosingForm = closestByTag(this.activeElement_, 'form');
    if (!enclosingForm) {
      user().error(
  ...
  onMessage_(event) {
    if (event.data.message === 'loadPaymentData') {
      ...
      const customEvent = createCustomEvent(
          this.ampdoc_.win, `payment-google-inline.${name}`, event.data.data);
      this.actions_.trigger(
          this.activeElement_, name, customEvent, ActionTrust.HIGH);
  ...
  getPaymentTokenInput_() {
    ...
    const input = this.ampdoc_.win.document.getElementById(
        paymentTokenInputId);
    if (!input) {
      user().error(
          TAG,
